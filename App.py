...
# ====================== CARREGAMENTO E DEDUPLICAÇÃO DO PARQUET ======================
@st.cache_data
def carregar_dados():
    df = pd.read_parquet("industrias_rs_efact.parquet")
    
    # 🛡️ Blindagem estrita contra duplicação de CNPJ
    ...
    # Padronização e limpeza tolerante do porte
    def limpar_porte(val):
        if pd.isna(val):
            return 'Outros / Não Informado'
        v_str = str(val).strip().split('.')[0].zfill(2)
        mapeamento = {
            '01': 'Não Aplicável',
            '03': 'Microempresa (ME)',
        ...
        return mapeamento.get(v_str, 'Outros / Não Informado')

    if "porte" in df.columns and "porte_descricao" not in df.columns:
        df["porte_descricao"] = df["porte"].apply(limpar_porte)
    elif "porte_descricao" in df.columns:
        df["porte_descricao"] = df["porte_descricao"].fillna('Outros / Não Informado')
    else:
        df["porte_descricao"] = 'Outros / Não Informado'
...
if not df_filtrado.empty:
    lat_centro = float(df_filtrado["latitude"].mean())
    lon_centro = float(df_filtrado["longitude"].mean())

mapa_efact = folium.Map(location=[lat_centro, lon_centro], zoom_start=7, tiles="CartoDB positron")
...
st_folium(mapa_efact, use_container_width=True, height=600, returned_objects=[])
