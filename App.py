...
# 1. AUTENTICAÇÃO E SANEAMENTO ROBUSTO DA CHAVE PRIVADA
if "gcp" in st.secrets:
    creds_dict = dict(st.secrets["gcp"])
    
    # Tratamento ultra-resiliente para a chave privada (remove aspas extras, ajusta quebras e limpa espaços)
    if "private_key" in creds_dict:
        pk = str(creds_dict["private_key"]).strip(' \'"')
        pk = pk.replace("\\n", "\n")
        lines = [line.strip() for line in pk.split("\n") if line.strip()]
        # Garante a estrutura correta do formato PEM
