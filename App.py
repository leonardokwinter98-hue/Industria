...
# 2. CARREGAMENTO DOS DADOS VIA ARQUIVO ZIP
@st.cache_data
def carregar_dados():
    nome_zip = "dados_industrias_rs.zip"
    nome_csv = "dados_industrias_rs.csv"
    
    # Se o arquivo zip existir na pasta do repositório, extrai o CSV
    if os.path.exists(nome_zip):
        with zipfile.ZipFile(nome_zip, 'r') as zip_ref:
            zip_ref.extractall(".")
            
    # Lê o CSV descompactado (ou direto se já estiver extraído)
    if os.path.exists(nome_csv):
        df = pd.read_csv(nome_csv)
        return df
    else:
        raise FileNotFoundError(f"O arquivo '{nome_zip}' ou '{nome_csv}' não foi encontrado no repositório.")
