...
# 2. EXTRAÇÃO REAL E DIRETA DO BIGQUERY (SEM FALLBACKS FICTÍCIOS)
...
def carregar_dados_reais_bigquery():
    ...
    # Pega o ID do projeto configurado nos segredos do Streamlit Cloud
    billing_id = st.secrets["gcp"]["billing_project_id"]
    
    # Executa a query real no BigQuery
    df = bd.read_sql(query, billing_project_id=billing_id)
    
    # Mapeamento do porte oficial da Receita Federal
    mapeamento_porte = {
        '01': 'Não Aplicável',
        '03': 'Microempresa (ME)',
    ...
    # Classificação dos subsetores eFact com base no CNAE
...
try:
    with st.spinner("Executando consulta oficial no BigQuery (Base dos Dados) para todo o RS... Isso pode levar alguns segundos na primeira carga."):
        df = carregar_dados_reais_bigquery()
except Exception as e:
    st.error(f"❌ Erro ao conectar com o BigQuery: {e}")
    st.warning("⚠️ **Atenção:** Para extrair dados reais direto do BigQuery no Streamlit Cloud, você precisa configurar o seu ID de projeto do Google Cloud nos **Secrets** do aplicativo.")
    st.stop()
...
for _, row in df_amostra.iterrows():
    ...
    folium.CircleMarker(
        ...
        fill_opacity=0.8,
        popup=folium.Popup(html_popup, max_width50),
        tooltip=f"{nome} ({mun})",
