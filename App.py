...
# 1. AUTENTICAÇÃO E SANISETIZAÇÃO AUTOMÁTICA DA CONTA DE SERVIÇO
if "gcp" in st.secrets:
    creds_dict = dict(st.secrets["gcp"])
    
    # Corrige e limpa eventuais espaços ou quebras incorretas na chave privada do TOML
    if "private_key" in creds_dict:
        pk = creds_dict["private_key"]
        pk = pk.replace("\\n", "\n")
        # Remove espaços em branco nas extremidades de cada linha causados por recuos do TOML
...
st.markdown(
    "Painel executivo com motor de extração em tempo real via **BigQuery** — Abrangência Estadual (RS)."
)

# 3. EXTRAÇÃO DIRETA COM O CLIENTE OFICIAL DO BIGQUERY
...
# 7. TABELA E EXPORTAÇÃO DE DADOS
