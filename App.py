...
def carregar_dados():
    ...
    # Se o CSV já estiver descompactado no repositório, lê direto
    if os.path.exists(nome_csv):
        return pd.read_csv(nome_csv)
    
    # Se houver um arquivo zip, tenta extrair o CSV de dentro dele
    elif os.path.exists(nome_zip):
        with zipfile.ZipFile(nome_zip, 'r') as zip_ref:
            zip_ref.extractall(".")
        if os.path.exists(nome_csv):
            return pd.read_csv(nome_csv)
            
    # Procura por qualquer arquivo CSV na pasta caso o nome seja diferente
    for file in os.listdir("."):
        if file.endswith(".csv"):
            return pd.read_csv(file)
            
    raise FileNotFoundError("Nenhum arquivo de dados (.csv ou .zip) foi encontrado no repositório do GitHub.")
