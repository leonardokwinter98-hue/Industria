...
def setup_gcp_credentials():
    if "gcp" not in st.secrets:
        st.error("❌ Seção 'gcp' não encontrada em st.secrets")
        st.stop()
    
    creds_dict = dict(st.secrets["gcp"])
    
    if "private_key" not in creds_dict:
        st.error("❌ Campo 'private_key' não encontrado nas secrets")
        st.stop()
    
    # Tratamento robusto da private_key
    pk = str(creds_dict["private_key"]).strip()
    pk = pk.replace("\\n", "\n").strip('"\' ')
    
    lines = [line.strip() for line in pk.splitlines() if line.strip()]
    
    # Garante formato PEM correto
    if not any("BEGIN PRIVATE KEY" in line for line in lines):
        lines.insert(0, "-----BEGIN PRIVATE KEY-----")
    if not any("END PRIVATE KEY" in line for line in lines):
        lines.append("-----END PRIVATE KEY-----")
    
    creds_dict["private_key"] = "\n".join(lines)
    
    # Salva arquivo temporário (backup)
